...
document.addEventListener('_AAEventForPageFramework', function(e) {
    //console.log('received', e.detail);
    PageFramework.documentFrameworkInfo = e.detail;
    ...
    if (!PageFramework.activePageFramework) {
        for (let registeredFramework of PageFramework.registeredPageFrameworks) {
            if (registeredFramework.CheckActive(PageFramework.documentFrameworkInfo)) {
                PageFramework.activePageFramework = pageFramework;
                break;
            }
        }
    }
...
var scriptText = ("(function() {" +
    " document.dispatchEvent(new CustomEvent('_AAEventForPageFramework', {" +
    "  detail: { workday: (typeof window.workday !== 'undefined'), salesforce: (typeof window.sfdc !== 'undefined'), sapfiori: (typeof window.sap !== 'undefined')  }" +
    " }));" +
    "})();");
...
function populateListViewChildren(htmlObject, listItemElements) {
    ...
    htmlObject.ItemCollection = htmlCommon.ReplaceSpacialCharecter(htmlObject.GetStringFromItemCollection(items));
}

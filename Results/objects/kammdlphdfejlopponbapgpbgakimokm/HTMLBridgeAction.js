...
function AALog(message) { try{console.log(message); } catch(e){} }
...
function ExecuteBridgeAction(inputString, browser, sendResponse)
{
    ...
    try
    {
        ...
        return executeCommand(requestObj, eResult, browser, sendResponse);
    }
    catch (e) {
        AALogger('ExecuteBridgeAction', 'ExecuteBridgeAction', e.message);
    ...
    switch (requestObj.RequestAction) {
        case HTMlRequestAction.PLAY_OBJECT_ACTION:
        {
            ...
            if (htmlElement == null || htmlElement.toString().toLowerCase().indexOf("htmlhtmlelement")!=-1 || htmlElement.toString().toLowerCase().indexOf("htmldocument")!=-1)  {
                ...
                //var htmlObject = new HTMLCommon().GetHtmlObject(requestObj.ParentPoint, htmlElement, requestObj.RequestAction );
                //eResult.SetHTMLObject(htmlObject);
            }
...
function WindowAvatarStringNotifier(bufferSize){
    ...
    this.SendFinalData = function () {
        if (!String.IsNullOrEmpty(_currentContent)) {
            chrome.runtime.sendMessage({ data: _currentContent, type: AVATAR_STRING });
            _currentContent = String.Empty;
        }
    }
};
...
function setFocus(htmlElement, DOMLocation) {
    try {
        ...
        AALogger('HTMLBridgeAction', 'setFocus', e.message);
    }
    try {
        ...
        AALogger('HTMLBridgeAction', 'executeCommand', e.message);
    }
}

function SetAccessibitySettings(version) {
            AccessibilitySettings.SearchVersion = xPathDetectionVersion.JavaScriptDOMXPath;
    AALogger('HTMLBridgeAction', 'SetAccessibitySettings', "Version : " + AccessibilitySettings.SearchVersion);
}
"";

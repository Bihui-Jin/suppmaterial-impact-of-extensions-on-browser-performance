            ...
            if (tagName == "html") {
                ...
                if (element && element.id) {
                    if (!attribValHasNumber(element.id)) {
                        ...
                        //if (index == 0) {
                        //    relativePath = tagName + "[1]/" + relativePath;
                        //} else {
...
HTMLRecorder = function () {
    ...
    function getElementFromPoint(requestObject, currentDocument, parentFrameInfo) {
        ...
        if (isUndfinedElement(recordedElement)) {
    ...
    function getCrossDomainFrameIndex(framesrc, frameDOMXPath) {
        ...
        for (i = 0; i < framelist.length; i++) {
            if (framelist[i].src == framesrc) {
                ...
                var framePath = htmlCommon.GetDOMXPath();
    ...
    function getDocument(currentDocument) {

        return document;
    }

    function isUndfinedElement(element) {

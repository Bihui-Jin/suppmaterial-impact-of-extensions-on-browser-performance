...
if (!triggerListenerAdded) {
    ...
    console.log('chrome trigger listener added');
} else {
    console.log('aa chrome trigger content script already registered');
}
...
function elementClickEvent(event) {
    ...
    matchedEventIds.forEach((uiEventId) => {
        console.log(`Sending callback to content script for event id ${uiEventId}`);
        chrome.runtime.sendMessage({
            uiEventId: uiEventId
        });
    });
    var end = performance.now();
    console.log(`elementClickEvent: ${end - start} ms`);
}

var d, dName, dMonth, dDay, dYear, dTail; //current data
var mood, overMonth, lastMood;
var locked, locking, editing;
var tMonth, tDay, tYear; //temporary (hover or selected)
var lMonth, lDay; //l is lock
var defaultNote = ". . .";
...
function start() {
  ...
  chrome.storage.sync.get(['firstRun'], function(result) {
    if (result.firstRun == 2 || result.firstRun == 1) switchToLocal();
    else if (result.firstRun == 3) getSaved();
    else firstRun();
  });
}
...
function generateCalendar(year) {
  ...
  if (tYear === dYear) markTodaysCell(dMonth, dDay, year);
  else removeTodaysCell(year);
}

function removeTodaysCell(year) {
  $("#dayNum").remove();
  for (i = 0; i < year.length; i++)
    for (j = 0; j <= year[i].length; j++) reviveCell(i, j);
}

function markTodaysCell(month, day, year) { //marks todays cell w date & disables next cells
  var tempDay = day > 9 ? "" + day : "0" + day; //keeps numbering formats consistent
  $(tempDName(month, day)).append("<span id='dayNum'>" + tempDay + "</span>");
  for (i = month + 1; i < year.length; i++) //disables next month's cells
    for (j = 0; j <= year[i].length; j++) killCell(i, j);
}
...
//GENERATE LISTS FOR A CERTAIN YEAR
